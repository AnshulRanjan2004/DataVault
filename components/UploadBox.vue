    ...
    <form class="space-y-3">
      <div class="grid grid-cols-1 space-y-2">
            ...
            <div class="
              ...
              <div v-else class="text-center flex flex-col justify-around items-center">
                <h1 class="
                    ...
                    dark:text-gray-400
                  ">
                  Encrypting<span v-if="selectedFiles.length > 0">&nbsp;{{ Object.keys(encryptedFiles).length }}/{{
        ...
        <transition enter-active-class="transition-all delay-500 duration-300 ease-out"
          ...
          enter-to-class="opacity-100 transform scale-100" leave-class="opacity-100 transform scale-100"
          leave-to-class="opacity-0 transform scale-0">
          <div class="text-center mt-5" v-if="encrypting || selectedFiles.length > 0">
            <h4 class="
                text-sm
                font-bold
                text-gray-400
      ...
      <div>
        <button :disabled="selectedFiles.length < 1 || encrypting || uploading" :class="{
    ...
    async handleFileSelected() {
      ...
      for (var i = 0; i < this.selectedFiles.length; i++) {
        try {
          ...
          this.errorMsg = `Unable to encrypt file ${this.selectedFiles[i].fullPath || this.selectedFiles[i].name
